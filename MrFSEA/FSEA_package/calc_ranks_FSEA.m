function [ranks,tail] = calc_ranks_FSEA(data,group,method,ifabs,iftrank,direction)
...
m = size(data,1);
gr0 = group==0; n0 = sum(gr0); % control
gr1 = group==1; n1 = sum(gr1); % case
...
switch method
    case 'S2N'
        ...
        ranks(direction<0) = -ranks(direction<0); % for features where increase anticorrelates to direction of feature family
        tail = 'both';
    case 'ttest'
    ...
    case 'ratio'
        ...
        ranks(direction<0) = 1/ranks(direction<0); % for features where increase anticorrelates to direction of feature family
        tail = 'both';
    case 'diff'
    ...
    %     tail = 'right';
    case 'WAD'
        ranks = WAD(data,group,direction);
        tail = 'both';
    case 'FCROS'
        ranks = FCROS(data,group,direction);
        tail = 'both';
    case 'MWT'
        ranks = mwt(data,group,direction);
        tail = 'both';
    % case 'MSD'
    %     ranks = MSD(data,group);
    %     tail = 'right';
    case 'external'
        ranks = m_fun(data,group,direction);
        tail = 'both';
    otherwise
...
function WAD_stat = WAD(data,group,direction)
group_un = unique(group);   %find unique groups

%calculate average difference
dm_1 = mean(data(:,group==group_un(1)),2);
dm_2 = mean(data(:,group==group_un(2)),2);
AD = dm_2 - dm_1;
AD(direction<0) = -AD(direction<0); % for features where increase anticorrelates to direction of feature family
...
function res = FCROS(data,group,direction)
d1 = data(:,group==1);
n_d1 = sum(group==1);
...
rank(direction<0,:) = 1./rank(direction<0,:); % for features where increase anticorrelates to direction of feature family
rank = rank/n;
res = trimmean(rank,10,2);

function W = mwt(x,grp,direction)
grp_un = unique(grp); 
...
[s02,d0] = est_hyper(log(sew2),mean(dw,"omitnan"));
...
W(direction<0)=-W(direction<0); % for features where increase anticorrelates to direction of feature family
