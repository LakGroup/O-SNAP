function ranks_perm = pre_calc_ranks_FSEA(data_ranks,data,group,opts)
...
group_p(:) = {group};
perm_nb_p = num2cell(get_OSNAP_n_samples_per_split(opts.perm_nb,maxNumCompThreads));
...
elseif strcmp(opts.perm_type,'pheno')
    parfor p=1:maxNumCompThreads
        for a=1:perm_nb_p{p}
            ind = RandPermFast(n);
            ranks_perm_p{p}(:,a) = calc_ranks_FSEA(data,group_p{p}(ind),rank_type,if_abs,if_trank,direction);
        end
    end
end
