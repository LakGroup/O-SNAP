function percentile_data = plot_voronoi_density_percentiles(work_dir, groups, voronoi_data, percentile_data)
%% set up
n_groups = length(groups);
% create list of percentiles
percentile_fields = fieldnames(percentile_data{1});
percentile_fields = percentile_fields(3:end);
percentiles = cellfun(@(x) str2double(extract_after(x,'prctile_')),percentile_fields);
...
% get data
for g=1:n_groups
    g_y = strcmp(groups{g}, cellfun(@(x) x.group, percentile_data, 'uni', 0));
    n_samp_in_group = length(percentile_data{g_y}.sample_names);
    if n_samp_in_group > 0
        y_percentile{g_y} = cell2mat(cellfun(@(x) (percentile_data{g_y}.(x)), percentile_fields, 'uni',0))';
    end
end
...
for k=1:2
    if k==1
        save_path = fullfile(work_dir,'Percentile_values.png');
        title_name = 'Log Voronoi density Percentiles';
    else
        save_path = fullfile(work_dir,'Percentile_values_outliers_removed.png');
        title_name = {'Log Voronoi density Percentiles','(Outliers Removed)'};
    end
    ...
    % plot the values
    for g=1:n_groups
        if size(group_idx{g},2) < 1
            continue;
        end
        % plot every sample
        g_y = strcmp(groups{g}, cellfun(@(x) x.group, percentile_data, 'uni', 0));
        x = reshape(repmat(1:n_percentiles,size(group_idx{g},2),1).',1,[]) + x_dif(g);
        if k==1
            for i=1:n_percentiles
                y_percentile{g_y}(:,i) = filloutliers(percentile_data{g_y}.(percentile_fields{i}),Na_n);
            end
        else
            for i=1:n_percentiles
                    y_percentile{g_y}(:,i) = percentile_data{g_y}.(percentile_fields{i});
            end
        end
        ...
        p.Annotation.Legend_information.Icon_display_style = 'off';
    end
    % finish plot
    legend('Location','northwest','FontSize',18);
    ax = gca;
    ax.XTick = 1:n_percentiles;
    ax.XTickLabels = arrayfun(@num2str, percentiles, 'uni', 0);
    ax.XAxis.FontSize = 16;
    ax.YTickLabels = arrayfun(@(elem) sprintf('10^{%d}',elem), ax.YTick,'uni',0);
    ax.YAxis.FontSize = 16;
    title(title_name,'FontSize',36)
...
if all(cellfun(@(x) ~isempty(x), y_percentile,'uni',1))
    %% find diff_density between Low_col_vI and High_col_vI
    group_a = 'Low_col_vI';
    group_b = 'High_col_vI';
    delta_density_P = zeros(length(percentile_thresh));
    for i=1:length(percentile_thresh)
        a_data = percentile_data{strcmp(cellfun(@(elem) elem.group, percentile_data, 'uni',0),group_a)};
        b_data = percentile_data{strcmp(cellfun(@(elem) elem.group, percentile_data, 'uni',0),group_b)};
    ...
    for i=1:size(delta_density_P,1)
        ...
        saveas(gcf,fullfile(work_dir,['voronoi_density_diff_ColVI_' num2str(percentile_thresh(i)) '.png']));
    end
    ...
    %     set(gca,'Units','normalized','OuterPosition',[0 0 1 1]);
    %     saveas(gcf,fullfile(work_dir,['voronoi_density_diff_passage_' num2str(percentile_thresh(i)) '.png']));
    % end
    ...
    %     set(gca,'Units','normalized','OuterPosition',[0 0 1 1]);
    %     saveas(gcf,fullfile(work_dir,['voronoi_density_diff_treatment_' num2str(percentile_thresh(i)) '.png']));
    % end
end
%% plot violin plot
for i=1:length(percentile_thresh)
