function plot_SNAP_FSEA(T_FSEA,group_ctrl,group_case,save_path,options)
arguments
    T_FSEA table
    group_ctrl string
    group_case string
...
if up_flag && down_flag
    ...
    subplot(1,2,1)
    plot_FSEA_axis(T_FSEA,group_case,"case_greater_than_ctrl",true,"alpha",options.alpha);
    %% feature value decrease
    subplot(1,2,2)
    plot_FSEA_axis(T_FSEA,group_ctrl,"case_greater_than_ctrl",false,"alpha",options.alpha);
elseif up_flag
...
function plot_FSEA_axis(T_FSEA,group,options)
arguments
    T_FSEA table
    group string = "";
