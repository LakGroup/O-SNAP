function venn_data = interpret_features_venn(T,feature_comparisons,options)
arguments
    T table
...
if n_groups == 3
    venn_data = interpret_features_venn_3(feature_comparisons, save_path, "plot", options.plot,"alpha",options.alpha,"fold_change_threshold",options.fold_change_threshold);
% elseif n_groups == 4
...
save(fullfile(save_path,"features_venn.mat"),"venn_data");
end
...
for i=1:n_group_pairs
    venn_data{i}.groups = group_pairs(i,:);
    if ~isempty(feature_comparisons{i}.feature_table)
        ...
        venn_data{i}.n_increase_features = size(venn_data{i}.increase_features_group_1,1);
        venn_data{i}.n_decrease_features = size(venn_data{i}.increase_features_group_2,1);
end
...
for i=1:(length(venn_data)-1)
    venn_data{i}.n_increase_features = size(venn_data{i}.increase_features_group_1,1) - venn_data{end}.n_increase_features;
    venn_data{i}.n_decrease_features = size(venn_data{i}.increase_features_group_2,1) - venn_data{end}.n_decrease_features;
end
for i=1:n_group_pairs
    group_pair_idx_1 = find(strcmp(group_pairs_string,group_pairs_pairs(i,1)));
    group_pair_idx_2 = find(strcmp(group_pairs_string,group_pairs_pairs(i,2)));
