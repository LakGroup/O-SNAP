...
%% Identify number of components to plot
% Plot 2D if only 2 dimensions, else plot 3D
% Return if not enough components
...
%% Organize marker appearance if there are multiple replicates
...
% Appearance: color = groups, no replicate information
...
% Appearance: color = groups, marker type = replicate
...
% Appearance: color = replicate, marker type = color
...
% Only plot PCA scores, no color-coding
else 
    if n_comps >=3
